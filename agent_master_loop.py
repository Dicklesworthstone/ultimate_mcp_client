"""
Supercharged Agent Master Loop - v3.2 (Tier 1 & 2 Integration - Complete)
==========================================================================
...
and **working memory context awareness**.
...
AGENT_STATE_FILE = "agent_loop_state_v3.2.json" # Updated state file version
AGENT_NAME = "Maestro-v3.2" # Updated agent name
...
CONTEXT_WORKING_MEMORY_LIMIT = 10 # Max working memories to include in prompt context
# Error Handling
MAX_CONSECUTIVE_ERRORS = 3
# --- Tool Names (Includes Tier 1 & 2) ---
...
# --- Agent Loop Class (Modified for Tier 1 & 2) ---
class AgentMasterLoop:
    """Supercharged orchestrator implementing Tier 1 & 2 UMS enhancements."""
    ...
    def __init__(self, mcp_client_instance: MCPClient, agent_state_file: str = AGENT_STATE_FILE):
        ...
        # Ensure context_id mirrors workflow_id initially if workflow_id exists
    ...
    async def initialize(self) -> bool:
        """Initializes loop state, loads previous state, verifies client setup, including Tier 1 & 2 tools."""
        self.logger.info("Initializing agent loop...", emoji_key="gear")
        await self._load_agent_state()
        # Ensure context_id is set if workflow_id exists after loading state
        if self.state.workflow_id and not self.state.context_id:
            self.state.context_id = self.state.workflow_id
        ...
        try:
            ...
            # Verify essential tools (Added Tier 1 & 2)
            essential_tools = [
            ...
            if missing_essential:
                ...
                # Allow proceeding but log severe warning
                # return False
    ...
    async def _save_agent_state(self):
        ...
        # Ensure nested defaultdict is converted correctly
    ...
    async def _load_agent_state(self):
        ...
        if not self.agent_state_file.exists(): self.logger.info("No previous agent state file found. Using default state."); self.state = AgentState(); return
        try:
                 ...
                 if field_info.name in state_data:
                     if field_info.name == "current_plan":
                         try: kwargs["current_plan"] = [PlanStep(**step_data) for step_data in state_data["current_plan"]]
                          ...
                          # Recreate defaultdict structure
                          ...
                          if isinstance(stats_dict, dict):
                              for k, v in stats_dict.items():
                                  if isinstance(v, dict):
                                       recreated_stats[k] = {"success": v.get("success", 0), "failure": v.get("failure", 0), "latency_ms_total": v.get("latency_ms_total", 0.0)}
                          kwargs["tool_usage_stats"] = recreated_stats
                     else: kwargs[field_info.name] = state_data[field_info.name]
                 else:
                     if field_info.default_factory is not dataclasses.MISSING: kwargs[field_info.name] = field_info.default_factory()
    ...
    async def _gather_context(self) -> Dict[str, Any]:
        ...
        base_context = {
            ...
            "core_context": None,
            "current_working_memory": [], # Added for Tier 2
            "proactive_memories": [],
            "relevant_procedures": [],
        ...
        current_workflow_id = self.state.workflow_stack[-1] if self.state.workflow_stack else self.state.workflow_id
        current_context_id = self.state.context_id # Use separate context ID
        ...
        if current_context_id and self._find_tool_server(TOOL_GET_WORKING_MEMORY):
             try:
                 # Use context_id to fetch working memory
                 wm_result = await self._execute_tool_call_internal(
                     TOOL_GET_WORKING_MEMORY,
                     {"context_id": current_context_id, "include_content": False, "include_links": False},
                     record_action=False
                 )
                 if wm_result.get("success"):
                     wm_mems = wm_result.get("working_memories", [])
                     base_context["current_working_memory"] = [
                         {
                             "memory_id": m.get("memory_id"), "description": m.get("description"),
                             "type": m.get("memory_type"), "importance": m.get("importance")
                         } for m in wm_mems[:CONTEXT_WORKING_MEMORY_LIMIT] # Limit count
                     ]
                     self.logger.info(f"Retrieved {len(base_context['current_working_memory'])} items from working memory for context {current_context_id}.")
        ...
        if self._find_tool_server(TOOL_HYBRID_SEARCH):
            try:
                result_content = await self._execute_tool_call_internal(
                    TOOL_HYBRID_SEARCH, {
                        "workflow_id": current_workflow_id, "query": proactive_query,
                        "limit": CONTEXT_PROACTIVE_MEMORIES, "include_content": False,
                ...
                if result_content.get("success"):
                    ...
                    base_context["proactive_memories"] = [
                        {"memory_id": m.get("memory_id"), "description": m.get("description"), "hybrid_score": m.get("hybrid_score"), "type": m.get("memory_type")}
                        for m in proactive_mems
                    ]
                    if base_context["proactive_memories"]: self.logger.info(f"Retrieved {len(base_context['proactive_memories'])} proactive memories using hybrid search.")
                else: base_context["errors"].append(f"Proactive hybrid memory search failed: {result_content.get('error')}")
            except Exception as e: self.logger.warning(f"Proactive hybrid memory search exception: {e}"); base_context["errors"].append(f"Proactive hybrid search exception: {e}")
        # Fallback to semantic if hybrid fails or unavailable
        elif self._find_tool_server(TOOL_SEMANTIC_SEARCH):
            self.logger.debug("Hybrid search unavailable, falling back to semantic for proactive memories.")
            try:
                result_content = await self._execute_tool_call_internal(TOOL_SEMANTIC_SEARCH, {"workflow_id": current_workflow_id, "query": proactive_query, "limit": CONTEXT_PROACTIVE_MEMORIES, "include_content": False}, record_action=False)
                if result_content.get("success"):
                    base_context["proactive_memories"] = [{"memory_id": m.get("memory_id"), "description": m.get("description"), "similarity": m.get("similarity"), "type": m.get("memory_type")} for m in result_content.get("memories", [])]
                else: base_context["errors"].append(f"Proactive semantic memory search failed: {result_content.get('error')}")
            except Exception as e: self.logger.warning(f"Proactive semantic memory search exception: {e}"); base_context["errors"].append(f"Proactive semantic search exception: {e}")
        else: self.logger.warning(f"Skipping proactive memory search: Tools '{TOOL_HYBRID_SEARCH}' and '{TOOL_SEMANTIC_SEARCH}' unavailable.")
             ...
             try:
                 ...
                 if proc_result.get("success"):
                     ...
                     base_context["relevant_procedures"] = [
                         {"memory_id": m.get("memory_id"), "description": m.get("description"), "hybrid_score": m.get("hybrid_score")}
                         for m in proc_mems
                     ]
                     if base_context["relevant_procedures"]: self.logger.info(f"Retrieved {len(base_context['relevant_procedures'])} relevant procedures using hybrid search.")
                 else: base_context["errors"].append(f"Procedure hybrid search failed: {proc_result.get('error')}")
             except Exception as e: self.logger.warning(f"Procedure hybrid search exception: {e}"); base_context["errors"].append(f"Procedure hybrid search exception: {e}")
        # Fallback for procedures if hybrid fails
        elif self._find_tool_server(TOOL_SEMANTIC_SEARCH):
            self.logger.debug("Hybrid search unavailable, falling back to semantic for procedures.")
            try:
                 proc_query = f"How to accomplish: {active_plan_step_desc}"
                 proc_result = await self._execute_tool_call_internal(TOOL_SEMANTIC_SEARCH, {"workflow_id": current_workflow_id, "query": proc_query, "limit": CONTEXT_PROCEDURAL_MEMORIES, "memory_level": MemoryLevel.PROCEDURAL.value, "include_content": False}, record_action=False)
                 if proc_result.get("success"): base_context["relevant_procedures"] = [{"memory_id": m.get("memory_id"), "description": m.get("description"), "similarity": m.get("similarity")} for m in proc_result.get("memories", [])]
                 else: base_context["errors"].append(f"Procedure semantic search failed: {proc_result.get('error')}")
            except Exception as e: self.logger.warning(f"Procedure semantic search exception: {e}"); base_context["errors"].append(f"Procedure semantic search exception: {e}")
        else: self.logger.warning(f"Skipping procedure search: Tools '{TOOL_HYBRID_SEARCH}' and '{TOOL_SEMANTIC_SEARCH}' unavailable.")
        ...
        if self._find_tool_server(get_linked_memories_tool):
            ...
            # Try finding a focus memory first from working memory, then important memories
            if base_context.get("current_working_memory"):
                # Simplistic: pick the first one. Could be smarter (e.g., most important in WM).
                if base_context["current_working_memory"]:
                    mem_id_to_traverse = base_context["current_working_memory"][0].get("memory_id")
            ...
            else: self.logger.debug("No important or working memory found to perform link traversal from.")
    ...
    # --- Prompt Construction (Updated for Tier 2 Tools & Context) ---
    def _construct_agent_prompt(self, goal: str, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Constructs the prompt for the LLM, including Tier 1 & 2 tools and instructions."""
...
"""
        ...
        # Add Tier 1 & 2 Instructions
        system_prompt += """
Your Process:
1.  Context Analysis: Deeply analyze 'Current Context'. Note workflow status, errors (`last_error_details`), recent actions, memories (`core_context`, `proactive_memories`), thoughts, `current_plan`, `relevant_procedures`, `current_working_memory` (most active memories), and `meta_feedback`. Pay attention to memory `importance`/`confidence`.
...
3.  Reasoning & Planning:
    a. State step-by-step reasoning towards the Goal/Sub-goal, integrating context and feedback. Consider `current_working_memory` for immediate context.
    b. Evaluate `current_plan`. Is it valid? Does it address errors? Are dependencies (`depends_on`) likely met?
    c. **Action Dependencies:** If planning Step B requires output from Step A (action ID 'a123'), include `"depends_on": ["a123"]` in Step B's plan object. The loop handles recording the dependency.
    ...
    e. **Direct Memory Management:** If you synthesize a critical new fact, insight, or piece of knowledge (e.g., from tool output, reasoning), plan to use `store_memory` to explicitly save it. If you find strong evidence contradicting a stored memory, plan to use `update_memory` to correct its content or lower its confidence. Provide clear `content`, `memory_type`, `importance`, and `confidence`.
    f. **Linking:** Identify potential memory relationships (causal, supportive, contradictory). Plan to use `create_memory_link` with specific `link_type`s.
    g. **Search:** Prefer `hybrid_search_memories` for finding relevant information if your query involves both concepts and specific keywords/filters. Use `search_semantic_memories` for purely conceptual similarity.
    h. Propose an **Updated Plan** (1-3 structured `PlanStep` JSON objects). Explain reasoning for changes. Use `record_thought(thought_type='plan')` for complex planning.
...
Key Considerations:
*   Use memory confidence. Trust high-confidence memories more. Use `update_memory` if confidence changes.
*   Explicitly store important learned information using `store_memory`.
*   Leverage `current_working_memory` for immediate relevance.
*   Dependencies: Ensure actions in `depends_on` are likely complete. Use `get_action_details` if unsure.
*   Artifacts: Track outputs using `record_artifact`. Retrieve using `get_artifacts`/`get_artifact_by_id`.
*   Linking: Create meaningful links using appropriate `link_type`s.
"""
        # Prepare context string
        context_str = json.dumps(context, indent=2, default=str, ensure_ascii=False); max_context_len = 25000 # Increased slightly
    ...
    async def _call_agent_llm(self, goal: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for block in response.content:
                ...
                elif block.type == "tool_use": tool_call = block # Prioritize tool call logic after parsing text
    ...
    async def _run_auto_linking(self, memory_id: str):
        ...
        try:
            ...
            # Use SEMANTIC search for auto-linking candidates for now, hybrid might be overkill here
            ...
            for similar_mem_summary in similar_results.get("memories", []):
                ...
                # Check if create_link tool is available before calling
                if not self._find_tool_server(TOOL_CREATE_LINK):
                     self.logger.warning(f"Cannot create link: Tool {TOOL_CREATE_LINK} unavailable.")
                     break # Stop trying to link if tool is missing
    ...
    async def _check_prerequisites(self, dependency_ids: List[str]) -> Tuple[bool, str]:
        ...
        try:
            ...
            found_ids = {a.get("action_id") for a in actions}; missing = list(set(dependency_ids) - found_ids)
    ...
    async def _execute_tool_call_internal(
        ...
        # Avoid recording internal memory operations like get_context or searches as primary actions unless necessary
        should_record_start = record_action and tool_name not in [
            TOOL_RECORD_ACTION_START, TOOL_RECORD_ACTION_COMPLETION,
            TOOL_GET_CONTEXT, TOOL_GET_WORKING_MEMORY, TOOL_SEMANTIC_SEARCH,
            TOOL_HYBRID_SEARCH, TOOL_QUERY_MEMORIES, TOOL_GET_MEMORY_BY_ID,
            TOOL_GET_LINKED_MEMORIES, TOOL_GET_ACTION_DETAILS, TOOL_GET_ARTIFACTS,
            TOOL_GET_ARTIFACT_BY_ID, TOOL_ADD_ACTION_DEPENDENCY, TOOL_CREATE_LINK,
            TOOL_LIST_WORKFLOWS, TOOL_COMPUTE_STATS, TOOL_SUMMARIZE_TEXT,
            TOOL_OPTIMIZE_WM, TOOL_AUTO_FOCUS, TOOL_PROMOTE_MEM, TOOL_REFLECTION,
            TOOL_CONSOLIDATION
        ]
        ...
        try:
            ...
            # Inject workflow_id automatically if missing
            ...
            # Inject context_id if needed by the tool (e.g., get_working_memory, optimize_wm)
    ...
    async def _record_action_dependencies_internal(self, source_action_id: str, target_action_ids: List[str]):
        ...
        dep_tasks = []
        unique_target_ids = set(target_action_ids)
        ...
        valid_target_ids = [tid for tid in unique_target_ids if tid != source_action_id] # Align results with targets
        for i, res in enumerate(results):
            if i >= len(valid_target_ids): break # Safety break if lengths mismatch
    ...
    async def _record_action_start_internal(self, primary_tool_name: str, primary_tool_args: Dict[str, Any], primary_target_server: str) -> Optional[str]:
         ...
         start_reasoning = f"Agent initiated tool: {primary_tool_name}"
         current_wf_id = self.state.workflow_stack[-1] if self.state.workflow_stack else self.state.workflow_id
         ...
         try:
             ...
             # Use _execute_tool_call_internal to call record_action_start, but disable recursive recording
             ...
             if start_result_content.get("success"):
                 ...
                 error_msg = start_result_content.get('error', 'Unknown')
    ...
    async def _handle_workflow_side_effects(self, tool_name: str, arguments: Dict, result_content: Dict):
        ...
        if tool_name == TOOL_CREATE_WORKFLOW and result_content.get("success"):
            ...
            if new_wf_id:
                self.state.workflow_id = new_wf_id; self.state.context_id = new_wf_id # Set context_id too
            ...
            if status in [s.value for s in [WorkflowStatus.COMPLETED, WorkflowStatus.FAILED, WorkflowStatus.ABANDONED]] and self.state.workflow_stack and wf_id == self.state.workflow_stack[-1]:
                 ...
                 if self.state.workflow_stack:
                      self.state.workflow_id = self.state.workflow_stack[-1]; self.state.context_id = self.state.workflow_id; self.logger.info(f"Sub-workflow {finished_wf} finished. Returning to parent {self.state.workflow_id}.", emoji_key="arrow_left"); self.state.needs_replan = True; self.state.current_plan = [PlanStep(description=f"Returned from sub-workflow {finished_wf} (status: {status}). Re-assess parent goal.")]
                 else: self.state.workflow_id = None; self.state.context_id = None; self.logger.info(f"Root workflow {finished_wf} finished."); self.state.goal_achieved_flag = True
    ...
    async def _update_plan(self, context: Dict[str, Any], last_decision: Dict[str, Any], last_tool_result_content: Optional[Dict[str, Any]] = None):
        ...
        if last_decision.get("decision") == "call_tool":
            ...
            if tool_success:
                 current_step.status = "completed"; action_id_from_result = last_tool_result_content.get('action_id') or (last_tool_result_content.get('data') or {}).get('action_id')
                 summary_text = f"Success: {str(last_tool_result_content)[:100]}..."
                 if action_id_from_result and last_decision.get("tool_name") == TOOL_RECORD_ACTION_START : summary_text += f" (ActionID: {action_id_from_result[:8]})" # Only add if it was the start action result
                 ...
                 self.state.consecutive_error_count += 1; self.state.needs_replan = True; self.state.successful_actions_since_reflection = self.reflection_threshold
    ...
    async def _run_periodic_tasks(self):
        ...
        required_attrs = ['reflection_type_sequence', 'consolidation_threshold', 'consolidation_memory_level', 'consolidation_max_sources', 'optimization_interval', 'promotion_interval', 'reflection_threshold']
        for attr in required_attrs:
            if not hasattr(self, attr): self.logger.error(f"Missing required configuration attribute: {attr}"); return
        ...
        if self.state.needs_replan or self.state.successful_actions_since_reflection >= self.reflection_threshold:
            if reflection_tool_available:
                reflection_type = self.reflection_type_sequence[self.state.reflection_cycle_index % len(self.reflection_type_sequence)]; tasks_to_run.append((TOOL_REFLECTION, {"workflow_id": self.state.workflow_id, "reflection_type": reflection_type})); trigger_reason.append(f"ReplanNeeded({self.state.needs_replan})" if self.state.needs_replan else f"SuccessCount({self.state.successful_actions_since_reflection})"); self.state.successful_actions_since_reflection = 0; self.state.reflection_cycle_index += 1
        ...
        if self.state.successful_actions_since_consolidation >= self.consolidation_threshold:
             if consolidation_tool_available: tasks_to_run.append((TOOL_CONSOLIDATION, {"workflow_id": self.state.workflow_id, "consolidation_type": "summary", "query_filter": {"memory_level": self.consolidation_memory_level}, "max_source_memories": self.consolidation_max_sources})); trigger_reason.append(f"ConsolidateThreshold({self.state.successful_actions_since_consolidation})"); self.state.successful_actions_since_consolidation = 0
    ...
    # --- Run Method (Main Loop - Incorporates Tier 1 & 2) ---
    async def run(self, goal: str, max_loops: int = 50):
        ...
        while not self.state.goal_achieved_flag and self.state.current_loop < max_loops:
             ...
             # 1. Gather Context (Enhanced for Tier 2)
             ...
             elif "errors" in context and context.get("errors"): # Check if errors list is not empty
                  self.logger.warning(f"Context gathering encountered errors: {context['errors']}. Proceeding cautiously.")
                  # Decide if errors are critical enough to pause - for now, proceed but log
                  # self.state.consecutive_error_count += 1; self.state.needs_replan = True
             ...
             if decision_type == "call_tool":
                 ...
                 if not tool_name: self.logger.error("LLM requested tool call but provided no tool name."); self.state.last_action_summary = "Agent error: Missing tool name."; self.state.last_error_details = {"agent_decision_error": "Missing tool name"}; self.state.consecutive_error_count += 1; self.state.needs_replan = True
                  ...
                  if self.state.workflow_id:
                      try: thought_result = await self._execute_tool_call_internal(TOOL_RECORD_THOUGHT, {"workflow_id": self.state.workflow_id, "content": thought_content, "thought_type": ThoughtType.INFERENCE.value}, True); assert thought_result.get("success")
...
if __name__ == "__main__":
    ...
    # --- Updated Goal for Tier 2 Testing ---
    AGENT_GOAL = os.environ.get("AGENT_GOAL",
        "Create workflow 'Tier 2 Test'. "
        "Plan: 1. Simulate asking user for their favorite color ('blue'). 2. Store this fact using store_memory (content='User favorite color is blue', type='fact', importance=7). 3. Search memory for favorite color using hybrid_search. 4. Simulate asking user for their favorite number ('42'). 5. Update the favorite color memory using update_memory (content='User favorite color is blue, number is 42', importance=7.5). 6. Verify update by getting memory by ID. Mark complete."
    )
    ...
    print(f"--- {AGENT_NAME} (Tier 1 & 2) ---") # Updated name
    ...
    async def simulate_ask_user(prompt: str):
        log.info(f"[SIMULATED] Asking user: {prompt}")
        await asyncio.sleep(0.2)
        if "color" in prompt.lower(): user_response = "blue"
        elif "number" in prompt.lower(): user_response = "42"
        else: user_response = "I don't know"
        log.info(f"[SIMULATED] User response: {user_response}")
        return {"success": True, "user_response": user_response}

    async def setup_and_run():
        """Wrapper to setup client and potentially register simulated tools."""
        # Placeholder: If MCPClient needs explicit registration, do it here.
        # Example:
        # client = MCPClient(...)
        # await client.register_tool_function("simulate:ask_user", simulate_ask_user)
        # await client.setup(...)
        # await run_agent_process(...) using this client instance

        # Run main process assuming client handles registration or tools are remote
