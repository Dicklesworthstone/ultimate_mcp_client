    ...
    *Only this class is fully implemented in this revision; the rest of the file
    ...
    async def auto_link(
        ...
        *,  # <- forces keyword for the override
        ...
        """
        link_type = (kind_hint or LinkKind(await self._infer_link_type(src_id, tgt_id, context_snip))).value
        ...
        await self.mcp_client._execute_tool_and_parse_for_agent(
            ...
            self._get_ums_tool_name("create_memory_link"),
            {
                ...
                "extra_json": json.dumps({"loop": self.state.loop_count, "phase": self.state.phase.value})[:300],
            },
        )
        ...
        try:
            ...
            if not contradictions_envelope.get("success"):
                self.mcp_client.logger.warning(
                    f"MCPClient: UMS get_contradictions failed: {contradictions_envelope.get('error_message', 'Unknown error')}"
                )
                return []
    ...
    async def promote_hot_memories(self, importance_cutoff: float = 7.5, access_cutoff: int = 5) -> None:
        ...
        try:
            ...
            candidates_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                ...
                {"workflow_id": self.state.workflow_id, "memory_level": "working", "min_importance": importance_cutoff, "limit": 100},
            )
    ...
    async def _infer_link_type(self, src_id: str, tgt_id: str, context: str) -> str:
        ...
        try:
            meta_src_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("get_memory_metadata"), {"workflow_id": self.state.workflow_id, "memory_id": src_id}
            )
            try:
            ...
            meta_tgt_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("get_memory_metadata"), {"workflow_id": self.state.workflow_id, "memory_id": tgt_id}
            )
            try:
    ...
    async def _get_memory_content(self, memory_id: str) -> str:
        try:
            res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("get_memory_by_id"), {"workflow_id": self.state.workflow_id, "memory_id": memory_id}
            )
            try:
    ...
    async def _get_tags_for_memory(self, memory_id: str) -> Set[str]:
        try:
            res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("get_memory_tags"), {"workflow_id": self.state.workflow_id, "memory_id": memory_id}
            )
            try:
    ...
    async def _get_cached_embedding(self, memory_id: str) -> Optional[list[float]]:
        ...
        try:
            ...
            res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("get_embedding"), {"workflow_id": self.state.workflow_id, "memory_id": memory_id}
            )
            try:
            ...
            create_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("create_embedding"), {"workflow_id": self.state.workflow_id, "memory_id": memory_id}
            )
            try:
    ...
    async def _cosine_similarity(self, vec_a: list[float], vec_b: list[float]) -> Optional[float]:
        ...
        try:
            ...
            res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("vector_similarity"), {"vec_a": vec_a, "vec_b": vec_b}
            )
            try:
    ...
    async def decay_link_strengths(self, half_life_days: int = 14) -> None:
        ...
        try:
            ...
            await self.mcp_client._execute_tool_and_parse_for_agent(
                ...
                {"workflow_id": self.state.workflow_id, "half_life_days": half_life_days},
            )
        except Exception:
    ...
    async def _get_metadata(self, memory_id: str) -> Dict[str, Any]:
        ...
        try:
            res = await self.mcp_client._execute_tool_and_parse_for_agent(
                self.ums_server_name, self._get_ums_tool_name("get_memory_metadata"), {"workflow_id": self.state.workflow_id, "memory_id": memory_id}
            )
            try:
        ...
        for a, b in ((src_id, tgt_id), (tgt_id, src_id)):
            try:
                ...
                await self.mcp_client._execute_tool_and_parse_for_agent(
                    ...
                    {"workflow_id": self.state.workflow_id, "memory_id": a, "metadata": meta},
                )
            except Exception:
                continue
    ...
    async def maybe_reflect(self, turn_ctx: Dict[str, Any]) -> None:
        ...
        try:
            reflection_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME,
                "generate_reflection",
                {"workflow_id": self.state.workflow_id, "reflection_type": reflection_type, "recent_ops_limit": 30, "max_tokens": 800},
            )
        ...
        if self.state.loop_count % 5 == 0:  # Every 5 turns
            try:
                optimize_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    UMS_SERVER_NAME, "optimize_working_memory", {"context_id": self.state.workflow_id, "strategy": "balanced"}
                )
                try:
    ...
    async def _goal_completed(self) -> bool:
        ...
        try:
            ...
            goal_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME, self._get_ums_tool_name("get_goal_details"), {"goal_id": self.state.current_leaf_goal_id}
            )
            ...
            links_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME,
                "get_linked_memories",
                {"workflow_id": self.state.workflow_id, "memory_id": self.state.current_leaf_goal_id, "direction": "outgoing"},
            )
        ...
        for pair in contradictions:
            ...
            try:
                ...
                meta_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    UMS_SERVER_NAME, "get_workflow_metadata", {"workflow_id": self.state.workflow_id}
                )
                try:
                ...
                update_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    UMS_SERVER_NAME, "update_workflow_metadata", {"workflow_id": self.state.workflow_id, "metadata": current_meta}
                )
                try:
                    ...
                    for mem in pair:
                        try:
                            tag_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                                UMS_SERVER_NAME,
                                "add_tag_to_memory",
                                {"workflow_id": self.state.workflow_id, "memory_id": mem, "tag": "BLOCKER"},
                            )
                            try:
    ...
    async def _validate_goal_outputs(self, goal_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            goal_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME, self._get_ums_tool_name("get_goal_details"), {"goal_id": goal_id}
            )
            ...
            if expects_artifact:
                ...
                artifacts_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    UMS_SERVER_NAME, "get_artifacts", {"workflow_id": self.state.workflow_id, "limit": 10, "is_output": True}
                )
                ...
                try:
                    ...
                    recent_artifacts = [a for a in artifacts if a.get("created_at", 0) >= goal_created]
            ...
            memories_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME, "get_linked_memories", {"workflow_id": self.state.workflow_id, "memory_id": goal_id, "limit": 20}
            )
            ...
            try:
                ...
                if resolution_mem_id:
                    ...
                    await self.mem_graph.auto_link(
                        src_id=mem_a, tgt_id=resolution_mem_id, context_snip="resolved contradiction", kind_hint=LinkKind.REFERENCES
                    )
                    await self.mem_graph.auto_link(
                        src_id=mem_b, tgt_id=resolution_mem_id, context_snip="resolved contradiction", kind_hint=LinkKind.REFERENCES
                    )
                ...
                try:
                    ...
                    await self.mcp_client._execute_tool_and_parse_for_agent(
                        ...
                        f"{UMS_SERVER_NAME}:record_action_completion",  # Use f-string
                        {
                            ...
                            "tool_result": result_envelope.get("data")
                            if success
                            else result_envelope,  # Store full envelope on error, data on success
                            "summary": summary,
        ...
        for i, call in enumerate(tool_calls):
            ...
            async def execute_with_timing(name=tool_name, args=tool_args, tid=tool_id):
                ...
                try:
                    ...
                    # Execute the tool
                    result = await self.mcp_client._execute_tool_and_parse_for_agent(server_name, name, args)
                    ...
                    if "individual_action_id" in locals() and individual_action_id:
                        try:
                            await self.mcp_client._execute_tool_and_parse_for_agent(
                                UMS_SERVER_NAME,
                                f"{UMS_SERVER_NAME}:record_action_completion",
                                {"action_id": individual_action_id, "status": "failed", "summary": f"Tool {name} failed with exception: {str(e)}"},
                            )
                        except Exception:
                            pass
...
class LLMOrchestrator:
    ...
    async def fast_structured_call(self, prompt: str, schema: dict[str, Any]):
        ...
        try:
            ...
            return await self.mcp_client.query_llm_structured(
                prompt_messages=[{"role": "user", "content": prompt}], response_schema=schema, model_override=fast_model, use_cheap_model=True
            )
        except Exception as e:
    ...
    async def big_reasoning_call(self, messages: list[dict], tool_schemas=None, model_name: str = None):
        ...
        if not valid_tool_names:
            ...
            return {"decision_type": "THOUGHT_PROCESS", "content": "No valid tools available. Cannot proceed with tool-based actions."}
        ...
        response_schema = {
            ...
            "properties": {
                ...
                "tool_calls": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "tool_args": {"type": "object", "properties": {}, "additionalProperties": False},
                ...
                "content": {"type": "string", "description": "Content for THOUGHT_PROCESS or DONE decisions"},
            },
        ...
        try:
            ...
            if isinstance(resp, dict):
                ...
                if resp.get("decision") == "TOOL_SINGLE" and resp.get("tool_name"):
                    return {"decision_type": "TOOL_SINGLE", "tool_name": resp["tool_name"], "tool_args": resp.get("tool_args", {})}
                elif resp.get("decision") == "PARALLEL_TOOLS" and resp.get("tool_calls"):
                    return {"decision_type": "TOOL_MULTIPLE", "tool_calls": resp["tool_calls"]}
                elif resp.get("decision") == "plan" and resp.get("updated_plan_steps"):
                    # Convert plan response to thought process
                    return {"decision_type": "THOUGHT_PROCESS", "content": f"Planning response with {len(resp['updated_plan_steps'])} steps"}
            ...
            if isinstance(resp, dict):
                ...
                if "tool" in resp and "tool_input" in resp:
                    ...
                    return {"decision_type": "TOOL_SINGLE", "tool_name": tool_name, "tool_args": tool_args}
                ...
                if "plan" in resp:
                    ...
                    return {"decision_type": "THOUGHT_PROCESS", "content": f"Planning response received: {resp}"}
...
class AgentMasterLoop:
                ...
                if cognitive_state_data.get("state_id"):  # Check for state_id within the UMS payload
                    ...
                    if persisted_current_goals_list_json:
                        try:
                            ...
                            persisted_current_goals_list = (
                                json.loads(persisted_current_goals_list_json)
                                if isinstance(persisted_current_goals_list_json, str)
                                else persisted_current_goals_list_json
                            )
                    ...
                    if isinstance(persisted_current_goals_list, list) and persisted_current_goals_list:
                        ...
                        self.logger.info(
                            f"Loaded current_leaf_goal_id from cognitive state: {self.state.current_leaf_goal_id[:8] if self.state.current_leaf_goal_id else 'None'}"
                        )
                        ...
                        self.logger.info(
                            f"No current_goals in loaded state, falling back to root_goal_id: {self.state.root_goal_id[:8] if self.state.root_goal_id else 'None'}"
                        )
                    else:
                        self.state.current_leaf_goal_id = None
                        self.logger.warning(
                            "No current_goals in loaded state and no root_goal_id available to fall back on for current_leaf_goal_id."
                        )
    ...
    async def _validate_and_reset_state(self) -> None:
        ...
        try:
            ...
            workflow_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME, self._get_ums_tool_name("get_workflow_details"), {"workflow_id": self.state.workflow_id}
            )
            ...
            if self.state.root_goal_id:
                goal_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    UMS_SERVER_NAME, self._get_ums_tool_name("get_goal_details"), {"goal_id": self.state.root_goal_id}
                )
        ...
        try:
            if make_focal:
                ...
                res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    ...
                    {"memory_id": memory_id, "context_id": self.state.context_id, "add_to_working": True},
                )
                ...
                wm_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                    ...
                    {"context_id": self.state.context_id, "include_content": False, "update_access": False},
                )
                    ...
                    if len(current_ids) > 20:
                        ...
                        try:
                            optimize_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                                ...
                                {"context_id": self.state.context_id, "target_size": 15, "strategy": "balanced"},
                            )
                ...
                for tool_name, tool_schema in list(sm.tools.items()):
                    try:
                        ...
                        if provider == "anthropic":
                            # Anthropic format
                            formatted_tool = {"name": sanitized_name, "description": description, "input_schema": input_schema}
                        else:
                            # OpenAI format (works for most providers)
                            formatted_tool = {
                                "type": "function",
                                "function": {"name": sanitized_name, "description": description, "parameters": input_schema},
                            }
    ...
    async def _get_valid_ums_data_payload(self, mcp_client_response: Dict[str, Any], tool_name_for_error_context: str) -> Dict[str, Any]:
        ...
        if not mcp_client_response.get("success"):
            ...
            self.logger.error(
                f"MCPClient communication layer failure for {tool_name_for_error_context}: {error_type} - {error_msg}. Full response: {mcp_client_response}"
            )
        ...
        if not isinstance(ums_tool_envelope, dict):
            self.logger.error(
                f"UMS tool {tool_name_for_error_context} response's outer 'data' field (expected UMS tool's envelope) is not a dict: {type(ums_tool_envelope)}. Full MCPClient data: {ums_tool_envelope}"
            )
        ...
        if not ums_tool_envelope.get("success"):
            ...
            self.logger.error(
                f"UMS tool {tool_name_for_error_context} reported internal failure: {ums_error_type} - {ums_error_msg}. Details: {ums_details}. Full UMS envelope: {ums_tool_envelope}"
            )
        ...
        if not isinstance(actual_ums_data_payload, dict):
            self.logger.error(
                f"UMS tool {tool_name_for_error_context} successful response's inner 'data' field (actual payload) is not a dict: {type(actual_ums_data_payload)}. Full UMS envelope: {ums_tool_envelope}"
            )
            raise RuntimeError(
                f"Unexpected payload structure from {tool_name_for_error_context}: UMS tool's actual data payload is not a dictionary."
            )
            ...
            for original_mcp_name, tool_obj in server_manager.tools.items():
                ...
                if original_mcp_name.startswith(f"{UMS_SERVER_NAME}:") and hasattr(tool_obj, "input_schema"):
                    ...
                    if llm_seen_name_for_this_ums_tool:
                        ...
                        self.logger.debug(
                            f"Added UMS tool schema for LLM name: '{llm_seen_name_for_this_ums_tool}' (Original: '{original_mcp_name}')"
                        )
                    else:
                        # This case should be rare if _format_tools_for_provider ran correctly
                        self.logger.warning(
                            f"Could not find LLM-seen name for UMS tool '{original_mcp_name}'. Schema not added for UMS structured output."
                        )
    ...
    async def _save_cognitive_state(self) -> None:
        ...
        try:
            ...
            mem_query_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME,
                self._get_ums_tool_name("query_memories"),
                {"workflow_id": self.state.workflow_id, "memory_level": "working", "limit": 20, "sort_by": "created_at", "sort_order": "DESC"},
            )
            ...
            focus_memory_ids = [mem["memory_id"] for mem in memories if mem.get("memory_id") and mem.get("importance", 0) >= 7.0][
                :3
            ]  # Limit to top 3
            ...
            try:
                actions_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                    UMS_SERVER_NAME, self._get_ums_tool_name("get_recent_actions"), {"workflow_id": self.state.workflow_id, "limit": 10}
                )
            ...
            if self.state.root_goal_id and self.state.root_goal_id != self.state.current_leaf_goal_id:
    ...
    async def _create_checkpoint(self, reason: str) -> Optional[str]:
        ...
        try:
            ...
            working_memory_res = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME, self._get_ums_tool_name("get_working_memory"), {"context_id": self.state.context_id}
            )
    ...
    async def run_main_loop(self, overall_goal: str, max_mcp_loops: int) -> bool:
        ...
        try:
            ...
            llm_decision = await self.llms.big_reasoning_call(messages, tool_schemas, model_name=model_name_for_llm_call)
            ...
            try:
                ...
                wf_resp_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                    ...
                    self._get_ums_tool_name("create_workflow"),
                    {
                        ...
                        "tags": ["agent-driven"],
                        "metadata": {"agent_version": "1.0", "created_by": "AgentMasterLoop", "start_time": int(time.time())},
                    },
                )
                ...
                goal_resp_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                    ...
                    self._get_ums_tool_name("create_goal"),
                    {
                        ...
                        "metadata": {"created_by": "AgentMasterLoop", "is_root_goal": True},
                    },
                )
                ...
                initial_memory_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                    ...
                    self._get_ums_tool_name("store_memory"),
                    {
                        ...
                        "context_data": {"phase": self.state.phase.value, "loop": self.state.loop_count},
                    },
                )
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(goal_data_envelope, dict) and isinstance(goal_data_envelope.get("goal"), dict):
                        ...
                        self.logger.info(
                            f"Root goal verified: {goal_actual_data.get('title', 'Unknown Goal Title')} - {goal_actual_data.get('description', 'No Goal Description')[:100]}"
                        )
                    else:
    ...
    async def _gather_context(self) -> Dict[str, Any]:
        ...
        try:
            ...
            has_working_memory = (
                working_memory_check.get("success")
                and working_memory_check.get("data", {}).get("working_memories")
                and len(working_memory_check["data"]["working_memories"]) > 0
            )
            ...
            rich_res_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                UMS_SERVER_NAME, self._get_ums_tool_name("get_rich_context_package"), params
            )
            ...
            try:
                ...
                if not context_package or not context_package.get("core_context"):
                    self.logger.error(
                        f"Rich context package from UMS is empty or missing core_context. UMS Tool Payload: {str(ums_tool_data_payload)[:500]}"
                    )
                ...
                try:
                    fallback_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                        UMS_SERVER_NAME,
                        self._get_ums_tool_name("get_working_memory"),
                        {"context_id": self.state.context_id, "include_content": True, "update_access": True},
                    )
            ...
            return {
                "rich_context_package": {
                    "retrieval_timestamp_ums_package": _dt.datetime.utcnow().isoformat(),
                    "core_context": {"workflow_id": self.state.workflow_id, "workflow_goal": "Task in progress", "workflow_status": "active"},
                ...
                "context_sources": {"rich_package": False, "compression_applied": False, "fallback_used": True, "error": True},
            }
    ...
    async def _maybe_spawn_fast_tasks(self, ctx: Dict[str, Any]) -> None:
        ...
        if len(working_memories) >= 3:
            ...
            for mem in working_memories[-5:]:  # Last 5 memories
                ...
                content = (
                    mem.get("content")
                    or mem.get("content_preview")
                    or mem.get("description", "")
                    or f"Memory type: {mem.get('memory_type', 'unknown')}, importance: {mem.get('importance', 0)}"
                )
            ...
            if len(memory_contents) >= 2 and all(len(content.strip()) > 10 for content in memory_contents):
                ...
                self.logger.debug(
                    f"Insufficient working memory content for insight generation: {len(memory_contents)} memories with meaningful content"
                )
...
Avoid circular reasoning. Each action should move closer to the goal."""
        ...
        try:
            ...
            if not self.tool_schemas:
                with console.capture() as capture:
                    console.print(
                        Panel(Text("❌ NO TOOL SCHEMAS AVAILABLE AT ALL!", style="bold red"), title="🔧 Tool Schema Status", border_style="red")
                    )
            ...
            with console.capture() as capture:
                console.print(
                    Panel(
                        Text(
                            f"📊 Total available: {len(self.tool_schemas)}\n"
            ...
            if not ranked_tools:
                with console.capture() as capture:
                    console.print(
                        Panel(
                            Text(
                                f"⚠️ Tool ranking returned EMPTY for goal: {goal_desc[:60]}...\n🔄 Falling back to first 15 tools", style="bold yellow"
                            ),
            ...
            with console.capture() as capture:
                ...
                console.print(
                    Panel(
                        Text(
                            f"✅ Ranked {len(ranked_tools)} tools for goal\n🏆 Top 5: {', '.join(tool_names)}\n📖 Goal: {goal_desc[:60]}...",
                            style="green",
                        ),
            ...
            try:
                ...
                with console.capture() as capture:
                    console.print(
                        Panel(Text(f"💥 ERROR in _get_tool_schemas: {str(e)}", style="bold red"), title="🔧 Tool Schema Error", border_style="red")
                    )
                self.logger.error(f"Tool schema error:\n{capture.get()}")
            except Exception:
    ...
    async def _handle_file_access_error(self, tool_name: str, tool_args: Dict[str, Any], error_message: str) -> bool:
        ...
        is_file_error = any(
            ...
            for kw in ["permission denied", "not found", "access denied", "no such file or directory", "os error", "disk", "ioerror"]
        )
    ...
    async def _enact(self, decision: Any) -> bool:
        ...
        if decision_type == "TOOL_SINGLE":
                ...
                if "get_artifact_by_id" in tool_name.lower() and result.get("data"):
                    ...
                    if goals_found:
                        ...
                        for i, goal in enumerate(goals_found[:5]):  # Limit for memory content to keep it concise
                            content_to_store += (
                                f"- ID: {goal.get('goal_id', '')[:8]}, Status: {goal.get('status', 'N/A')}, Title: {goal.get('title', '')[:50]}...\n"
                            )
                    ...
                    if consolidated_content and stored_mem_id:
                        self.state.last_action_summary = (
                            f"Successfully consolidated memories ({consolidation_type}) into new memory {stored_mem_id[:8]}"
                        )
            ...
            mem_id = await self._store_memory_with_auto_linking(
                content=thought, memory_type="reasoning_step", memory_level="working", importance=6.0, description="Reasoning from SMART model"
            )
            ...
            if link_to_goal and self.state.current_leaf_goal_id and memory_id:
                try:
                    ...
                    metadata_envelope = await self.mcp_client._execute_tool_and_parse_for_agent(
                        UMS_SERVER_NAME,
                        self._get_ums_tool_name("update_memory_metadata"),
                        {"workflow_id": self.state.workflow_id, "memory_id": memory_id, "metadata": current_meta},
                    )
